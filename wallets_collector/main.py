...
query = {"query": {"match_all": {}}, "sort": [{"balanceNum": {"order": "desc"}}], "track_total_hits": True}
...
for i, wallet in enumerate(accounts):
    ...
    query = {"query": {"bool": {"should": [{"match": {e: wallet}} for e in ["sender", "receiver", "receivers"]]}}, "sort": [{"timestamp": {"order": "desc"}}], "track_total_hits": True}
    ...
    ids = [e["_id"] for e in data if e["_source"].get("hasScResults") or e["_source"].get("hasOperations") or e["_source"].get("hasLogs")]
    ...
    query = {"query": {"bool": {"should": [{"terms": {"originalTxHash": ids}}]}}, "track_total_hits": True}
